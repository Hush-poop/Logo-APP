...
const showEvents = (event, id) => {
  const {name, email, Theme, status, description, booked} = event
...
const showLatestEvent = (latestEvent, id) => {
  ...
  const {name, email , Theme, status, description, booked} = latestEvent 
  // Get the first event
    welcomeEvent.innerHTML = `
    <h1>${name}</h1>
    <p>${description.length >= 100 ? `${description.substring(0, 100)}...` : description}</p>
    <div>
      <span>email: ${email - booked}</span>
